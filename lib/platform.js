...
function isProcessRunning(name) {
    ...
    try {
        ...
        execSync(`pgrep -x "${baseName}"`, { stdio: 'ignore', timeout: 3000 });
        return true;
    } catch (e) { return false; }
}
...
function killProcess(name) {
    if (IS_WIN) {
        ...
        try {
            const { execSync } = require('child_process');
            const baseName = name.replace(/\.\w+$/, '');
            execSync(`pkill -x "${baseName}"`, { stdio: 'ignore', timeout: 3000 });
        } catch (e) { /* proses mungkin sudah mati */ }
    }
}
...
function preflightCheck() {
    ...
    checks.push({ name: 'Browser (Chromium)', ok: !!browser, detail: browser || 'Tidak ditemukan' });

    const py = findPython();
    checks.push({ name: 'Python', ok: !!py, detail: py || 'Tidak ditemukan' });

    const agDirExists = fs.existsSync(AG_DIR);
    checks.push({ name: 'AG Tools dir', ok: agDirExists, detail: AG_DIR });

    const accountsDirExists = fs.existsSync(ACCOUNTS_DIR);
    checks.push({ name: 'Accounts dir', ok: accountsDirExists, detail: ACCOUNTS_DIR });
